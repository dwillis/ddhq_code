...
previous_emails <- read_csv("~/code/ddhq/emails.csv") |> filter(!is.na(party)) |> select(name, subject, party) |> distinct()
...
recent_needs_party <- emails_no_party |> filter(year > 2023, disclaimer == TRUE) |>  distinct(name, subject)

write_csv(recent_needs_party, "~/code/ddhq_code/recent_needs_party2.csv")

recent_needs_party_updated <- read_csv("~/code/ddhq_code/recent_needs_party2.csv") |> filter(!is.na(party))

emails_party <- emails %>% filter(!is.na(party))

emails_no_party <- emails_no_party %>% left_join(recent_needs_party_updated, join_by(name, subject))

emails_combined <- bind_rows(emails_party, emails_no_party)

emails_combined <- emails_combined %>% select(-subject.x, -subject.y)

emails_since_2020 <- emails %>% filter(year > 2019, disclaimer == TRUE)
...
write_csv(emails, "~/code/ddhq/emails.csv")
