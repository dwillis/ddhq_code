...
# Dictionary to store domain-party mappings
domain_party_map = {}

# Republican domains
republican_domains = ['standupforliberty.net', 'rightusa.org', 'victory.donaldtrump.com', 'rightwave.org', 'bestamericanow.com', 'ourdefenseofamerica.com', 'saveamericamajority.com', 'makingamericathegreatest.com',
...
# Democratic domains
democratic_domains = ['dscc.org', 'hillaryclinton.com', 'emilyslist.org', 'democrats.org', 'betoorourke.com', 'jayinslee.com', 'elizabethwarren.com', 'e.markkelly.com', 'dccc.org', 'e.ronbegone.com', 'e.johnfetterman.com',
...
# Independent domains
independent_domains = ['teamkennedy.com', 'cornelwest2024.com']

# Add Republican domains to the map
for domain in republican_domains:
    domain_party_map[domain] = 'R'

# Add Democratic domains to the map
for domain in democratic_domains:
    domain_party_map[domain] = 'D'

# Add Independent domains to the map
for domain in independent_domains:
    domain_party_map[domain] = 'I'

# Write the mapping to a CSV file
with open('domain_party_mapping.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['domain', 'party'])  # Write header
    for domain, party in domain_party_map.items():
        writer.writerow([domain, party])
